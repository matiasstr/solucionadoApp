...
3. Consultar `ROADMAP.md`, la guía correspondiente en `docs/steps/` y los ADRs (0001–0011) antes de implementar.

Estado al 2026-09-23: **fases 1, 2 y 3 completas** y **P4-01 completo**: catálogo, comercios, historia de precios, seed DEMO, API pública, motor de promociones, búsqueda y comparación, las pantallas públicas `/buscar` y `/producto/[id]`, y la API privada `/shopping-routines` + `/inventory` con ownership (ADR 0011). Ver ADR 0008, 0009 y 0011, **docs/API.md** (formato estable de las respuestas, incluida la sección privada) y la sección "Pantallas de comparación" del README. Próximo paso: **P4-02 — onboarding, `/mis-compras`, `/mi-despensa` y preferencias** (web contra la API de P4-01). Producción: web https://tusofertas.vercel.app + API https://tusofertas-api.vercel.app + Supabase, desplegadas y verificadas el 2026-09-24 (ADR 0010; comandos en la sección Deploy de CONTINUAR.md). Producción no tiene dataset DEMO.
